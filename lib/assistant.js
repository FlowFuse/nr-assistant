    ...
    async init (RED, options = {}) {
        ...
        try {
            ...
            if (!completionsSupported) {
                ...
                // if modelUrl is not set, use the default model URL from this.options.url + 'assets/completions/model.json'
                completionsSettings.modelUrl = completionsSettings.modelUrl || new URL('assets/completions/model.json', this.options.url).href
                // if vocabularyUrl is not set, use the default vocabulary URL from this.options.url + 'assets/completions/vocabulary.json'
                completionsSettings.vocabularyUrl = completionsSettings.vocabularyUrl || new URL('assets/completions/vocabulary.json', this.options.url).href
    ...
    async _loadTensorFlowModel (url = this.options.completions.modelUrl) {
        const fetchFunc = async (url, options) => {
            ...
            return {
                ...
                arrayBuffer: async () => response.body.buffer,
                text: async () => response.body.toString(),
                json: async () => JSON.parse(response.body.toString()),
                bytes: async () => new Uint8Array(response.body.buffer),
                blob: async () => new Blob([response.body], { type: response.headers['content-type'] || 'application/octet-stream' }),
                type: 'basic', // Mimic the fetch API's Response type
                redirected: false // Mimic the fetch API's Response redirected property
            }
        }
        try {
            this._completionsModel = await this._tf.loadLayersModel(url, { fetchFunc })
        } catch (error) {
            console.error('Error loading TensorFlow layers model:', error)
            throw new Error(`Failed to load TensorFlow model: ${error.message}`, { cause: error })
        }
        if (!this._completionsModel) {
