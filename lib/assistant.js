...
class Assistant {
    ...
    async init (RED, options = {}) {
        ...
        try {
            ...
            let runtimeOptions
            try {
                const response = await this.got.get(new URL('settings', this.options.url).href, {
                    headers: {
                ...
                runtimeOptions = response.body
                if (!runtimeOptions?.inlineCompletions) {
                    RED.log.info('FlowFuse Assistant inline completions will not be available for this instance')
                }
            } catch (error) {
                // if the endpoint fails it is likely because the backend is not up-to-date and
                // does not support this endpoint. In this case, we default to disabling inline completions
                RED.log.info('FlowFuse Assistant inline completions will not be available for this instance')
            }
            ...
            const clientSettings = {
                ...
                inlineCompletionsEnabled: !!runtimeOptions?.inlineCompletions,
