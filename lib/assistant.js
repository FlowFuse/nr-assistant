...
class Assistant {
    ...
    initAdminEndpoints (RED) {
        ...
        RED.httpAdmin.get('/nr-assistant/deploy-policy', RED.auth.needsPermission('write'), function (req, res) {
            return assistant.handleGetDeployPolicyRequest(req, res)
        })
    ...
    /**
     * Handles GET requests to the /nr-assistant/deploy-policy endpoint.
     * Relays a live check to forge's /api/v1/assistant/deploy-policy so the editor
     * can find out, at the moment a deploy is being considered, whether the team
     * has opted in to agent-initiated deploys. Deliberately not cached - the point
     * is that disabling the team setting takes effect on the very next check.
     * @param {import('express').Request} req - The request object
     * @param {import('express').Response} res - The response object
     */
    async handleGetDeployPolicyRequest (req, res) {
        ...
        const url = `${this.options.url.replace(/\/$/, '')}/deploy-policy`
        try {
            const response = await this.got(url, {
                headers: {
            ...
            res.json({ status: 'ok', autoDeploy: !!response.body?.autoDeploy })
        } catch (error) {
            this.RED.log.warn(`nr-assistant: failed to check deploy policy: ${error.message}`)
            res.json({ status: 'ok', autoDeploy: false })
        }
    }
