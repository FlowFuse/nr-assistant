        ...
        try {
            ...
            try {
                const response = await this.got.get({
                    url: new URL('settings', this.options.url).href,
                    headers: {
                ...
                runtimeOptions = JSON.parse(response.body)
            } catch (error) {
                console.error(error) // TODO: probably remove this
                ...
                RED.log.info('FlowFuse Assistant Plugin runtime options endpoint is not supported')
            }
