...
class Assistant {
    ...
    async init (RED, options = {}) {
        ...
        try {
            ...
            const mcpEnabled = true // See issue #57
            try {
            ...
            const completionsEnabled = mcpEnabled && completionsFeatureEnabled && this.isEnabled
    ...
    async loadMCP () {
        ...
        async ({ flow, sourceNode, sourcePort }) => {
            if (!this._completionsModel) {
                throw new Error('TensorFlow model is not loaded')
            }
            const upstreamNodes = []
            if (flow && flow.length) {
            ...
            values.dispose()
            indices.dispose()
            const newNodes = predictedWords.map(word => ({ type: word })) // Create new nodes with the predicted types
            // special cases:
            ...
            if (flow && flow.length > 1) {
                ...
                const httpResponseSuggested = newNodes.some(n => n.type === 'http response')
                if (hasHTTP && !hasHTTPResponse && !httpResponseSuggested) {
                    newNodes.push({ type: 'http response', x: 0, y: 0 })
                }
                ...
                const linkOutSuggested = newNodes.some(n => n.type === 'link out')
                if (hasLinkIn && !hasLinkOut && !linkOutSuggested) {
                    newNodes.push({ type: 'link out', x: 0, y: 0 })
                }
                ...
                const joinSuggested = newNodes.some(n => n.type === 'join')
                if (hasSplit && !hasJoin && !joinSuggested) {
                    newNodes.push({ type: 'join', x: 0, y: 0 })
                }
            }
            const suggestions = newNodes.map(node => [node])
            return {
                structuredContent: {
